...
import { shopingData } from '../../../commondata/commondata';
  ...
  let containerRef:any = useRef(null);
	...
	useEffect(() => {
		if (local_varaiable == undefined) {
			...
			local_varaiable.filter((ele: { newprice: any; }) => {
				setPrice(Number(ele.newprice) + Price);
			})
		}
      ...
      <Col lg={12} md={12} sm={12} xl={8}>
        <Card className="cart">
          ...
          <Card.Body>
            ...
            <tbody>
                {FiltershopingData !== "" ? FiltershopingData.map((list: any) => (
                    <tr key={Math.random()}>
                        <td>
                        ...
                        <OverlayTrigger placement="top" overlay={<Tooltip >Wishlist</Tooltip>}><div className="btn text-secondary bg-secondary-transparent btn-icon py-1 me-2"><i className="bi bi-heart fs-16"></i></div></OverlayTrigger>
                        <Link to ='#' onClick={() => { handleRemove(list.id) }}><OverlayTrigger placement="top" overlay={<Tooltip>Delete</Tooltip>}><div className="btn text-danger bg-danger-transparent btn-icon py-1"><i className="bi bi-trash fs-16"></i></div></OverlayTrigger></Link>
                        </div>
                        </td>
                    </tr>
        ...
        <Card>
          ...
          <Card.Footer>
            <div className="btn-list d-flex justify-content-between flex-wrap">
              <Link to={`${import.meta.env.BASE_URL}ecommerce/shop/`} className="btn btn-primary"><i className="fa fa-arrow-left mx-1"></i>Continue Shopping</Link>
              <Link to={`${import.meta.env.BASE_URL}ecommerce/checkout/`} className="btn btn-success">Check out<i className="fa fa-arrow-right mx-1"></i></Link>
            </div>
          </Card.Footer>
        </Card>
      </Col>
