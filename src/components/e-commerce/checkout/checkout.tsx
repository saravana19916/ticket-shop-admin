...
import {Country} from '../../../commondata/selectdata';
import { Imagesdata } from '../../../commondata/commonimages';

interface CheckoutProps { }

const Checkout: FC<CheckoutProps> = ({ local_varaiable }:any) => {
  ...
  let checkOutData : any = [
    {
      id: 4,
      preview: Imagesdata("product4"),
      heading: 'Flower Pot for Home Decor',
      price1: '$438'
    },
    {
      id: 7,
      preview: Imagesdata("product6"),
  ...
  useEffect(() => {
    ...
    if (local_varaiable == undefined) {
      ...
      local_varaiable.filter((ele : any) => {
        setPrice(ele.price1);
      })
    }
  }, [local_varaiable]);
          ...
          {FiltercartData.map((idx : any) => (
            <div className="" key={Math.random()}>
              <div className="d-flex mt-1">
                <img className="avatar-xxl br-7" src={idx.preview} alt="img" />
                <div className="ms-3">
                  <h4 className="mb-1 fw-semibold fs-14"><Link to={`${import.meta.env.BASE_URL}ecommerce/productdetails/${idx.id}`}>{idx.heading}</Link></h4>
