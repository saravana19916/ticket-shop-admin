import React, { FC, useState } from "react"; // Added useState import
...
const SingleGuestListAccordion: FC<IProps> = ({ item, index }) => {
  const [count, setCount] = useState(item.count || 0); // Initialize count with item.count

  const handleIncrease = (event: React.MouseEvent) => {
    event.stopPropagation(); // Prevents accordion from opening/closing
        ...
        <div className="d-flex p-3 w-100">
          <div className="row align-items-center flex-fill gap-2">
            ...
            <div className="col-6 ms-auto col-lg-2 d-flex justify-content-around bg-light-gray rounded-pill px-3 py-2 mt-4">
              <MinusIcon
                className="w-4 h-4 me-1 mb-1px fw-bold cursor-pointer"
